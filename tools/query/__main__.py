...
def main():
    ...
    # Check if first arg looks like a question (not a command)
    if len(sys.argv) > 1 and sys.argv[1] not in ['query', 'search', 'decisions', '-h', '--help']:
        # Insert 'query' subcommand automatically
  ...
  python -m tools.query search "machine learning" --wiki-dir /path/to/wiki
    ...
    if not args.command:
        ...
        if len(sys.argv) == 1:
            parser.print_help()
            return 0
        # If command parsing failed, try help
        parser.print_help()
        return 1
